...
async function loadDeals() {
  try {
    ...
    // Populate brand filter and initialize listeners (wrapped in try-catch to prevent blocking)
    try {
      populateBrandFilter();
      initFilterListeners();
    } catch (err) {
      console.warn('Error initializing filters:', err);
    }
...
function applyFilters() {
  ...
  if (currentFilters.category !== 'all') {
    filtered = filtered.filter(deal => deal.category === currentFilters.category);
  }

  // Filter by brand
  if (currentFilters.brand !== 'all') {
    filtered = filtered.filter(deal =>
      deal.brand.toLowerCase() === currentFilters.brand.toLowerCase()
    );
  }
  ...
  if (currentFilters.priceRange !== 'all') {
    ...
    filtered = filtered.filter(deal =>
      deal.currentPrice >= min && deal.currentPrice <= max
    );
  }

  // Filter by minimum discount
  if (currentFilters.minDiscount > 0) {
    filtered = filtered.filter(deal => deal.discount >= currentFilters.minDiscount);
  }
  ...
  if (currentFilters.searchQuery) {
    ...
    filtered = filtered.filter(deal =>
      ...
      deal.categoryName.toLowerCase().includes(query)
    );
  }
...
// Populate brand filter from available brands
function populateBrandFilter() {
  const brandFilter = document.getElementById('brandFilter');
  if (!brandFilter) {
    console.warn('Brand filter element not found - skipping brand filter population');
    return;
  }
  ...
  console.log(`Brand filter populated with ${brands.size} brands`);
}
...
function initFilterListeners() {
  ...
  if (discountFilter) {
    discountFilter.addEventListener('change', function() {
      currentFilters.minDiscount = parseInt(this.value) || 0;
      applyFilters();
    });
  }
